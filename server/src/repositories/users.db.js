...
export const makeUsersDb = (db) => {
  // findOne, findAll, ...
  const findOne = (payload, attributes = { includes: [], excludes: [] }) =>
    new Promise(async (resolve, reject) => {
      try {
        const rows = await db.User.findOne({ where: payload, attributes });
  ...
  const findById = (id, attributes = { includes: [], excludes: [] }) =>
    new Promise(async (resolve, reject) => {
      try {
        const rows = await db.User.findByPk(id);
        if (!rows) {
          return resolve(null);
        }
    ...
    new Promise(async (resolve, reject) => {
      try {
        const { rows } = await db.User.findAndCountAll({
          attributes: Object.keys(db.User?.rawAttributes).filter(
            (key) => key !== "password" && key !== "access_token"
          ),
  ...
  const create = (payload) =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        const filterData = arrtributesHandler.filter(newUser?.dataValues, {
          excludes: ["password"],
        });
  ...
  const update = (payload, whereCondition = {}) =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        await db.User.update(order, { where: whereCondition });
  ...
  const deleteByConditions = (whereCondition) =>
    new Promise(async (resolve, reject) => {
      try {
        const recordDeleted = await db.User.destroy({ where: whereCondition });
